...
func TestCombined_HighLoadWithPanicRecovery(t *testing.T) {
	...
	topic := Topic("test.combined.highload.panic")
	...
	normalSubscriber := SubscriberFunc(func(ctx context.Context, msg Message) error {
	...
	normalSub, err := s.Subscribe(topic, normalSubscriber, 
		WithConcurrency(5),
		WithQueueGroup("normal"))
	require.NoError(t, err)
	defer normalSub.Stop()

	panicSub, err := s.Subscribe(topic, panicSubscriber,
		WithConcurrency(3), 
		WithQueueGroup("panic"))
	require.NoError(t, err)
	defer panicSub.Stop()
