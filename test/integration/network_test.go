...
func TestNetwork_ConnectionRecoveryAfterDisconnection(t *testing.T) {
	...
	topic := Topic("network.connection.recovery")
	
	received := make(chan Message, 50)
...
func TestNetwork_TimeoutHandlingInNetworkScenarios(t *testing.T) {
	s := CreateTestStream(t)
	requestTopic := Topic("network.timeout.request")
	
	// Set up slow responder to test timeouts
	slowResponder := SubscriberFunc(func(ctx context.Context, msg Message) error {
...
func TestNetwork_MessageDeliveryGuaranteesUnderNetworkIssues(t *testing.T) {
	s := CreateTestStream(t)
	topic := Topic("network.delivery.guarantees")
	
	received := make(chan Message, 200)
...
func TestNetwork_NetworkLatencyImpact(t *testing.T) {
	s := CreateTestStream(t)
	topic := Topic("network.latency.impact")
...
func TestNetwork_BandwidthConstraints(t *testing.T) {
	s := CreateTestStream(t)
	topic := Topic("network.bandwidth.constraints")
	...
	for sizeIndex, size := range messageSizes {
		...
		for i := 0; i < messagesPerSize; i++ {
			...
			msg := Message{
				...
				Data:  data,
				Headers: map[string]string{
					"Message-ID": fmt.Sprintf("size-%d-msg-%d", size, i),
...
func TestNetwork_ProtocolCompliance(t *testing.T) {
	s := CreateTestStream(t)
	topic := Topic("network.protocol.compliance")
...
func TestNetwork_MessageFormatConsistency(t *testing.T) {
	s := CreateTestStream(t)
	topic := Topic("network.message.format")
...
func TestNetwork_SubscriptionManagementAtProtocolLevel(t *testing.T) {
	...
	topics := []Topic{
		Topic(baseTopic + ".user.created"),
		Topic(baseTopic + ".user.updated"),
		Topic(baseTopic + ".user.deleted"),
		Topic(baseTopic + ".order.created"),
		Topic(baseTopic + ".order.completed"),
	}
	...
	subscriptions := make([]Subscription, len(topics))
	...
	for _, topic := range topics {
		for i := 0; i < messagesPerTopic; i++ {
			msg := Message{
				...
				Headers: map[string]string{
					"Topic":      string(topic),
					"Message-ID": fmt.Sprintf("%s-%d", string(topic), i),
				},
				Time: time.Now(),
			}
	...
	newTopic := Topic(baseTopic + ".dynamic.test")
	dynamicCount := int64(0)
	
	dynamicSubscriber := SubscriberFunc(func(ctx context.Context, msg Message) error {
