...
func TestSubscribe_NonExistentTopic(t *testing.T) {
	...
	sub, err := helpers.CreateTestStream(t).Subscribe(
		topic.Topic("test.subscribe.nonexistent.very.specific.topic"),
		sub.SubscriberFunc(func(_ context.Context, _ message.Message) error { return nil }))
...
func TestSubscribe_ClosedStream(t *testing.T) {
	...
	_, err := s.Subscribe(topic.Topic("test.subscribe.closed"), sub.SubscriberFunc(func(ctx context.Context, msg message.Message) error {
		return nil
	}))
...
func TestSubscribeJSON_ValidData(t *testing.T) {
	...
	require.NoError(t, s.PublishJSON(ctx, top, user))
...
func TestSubscribeJSON_InvalidJSON(t *testing.T) {
	...
	require.NoError(t, s.Publish(ctx, top, invalidMsg))
...
func TestSubscribeJSON_TypeMismatch(t *testing.T) {
	...
	require.NoError(t, s.PublishJSON(ctx, top, wrongType))
...
func TestSubscribeJSON_EmptyMessage(t *testing.T) {
	...
	require.NoError(t, s.Publish(ctx, top, emptyMsg))
...
func TestSubscribe_ConcurrentSubscribers(t *testing.T) {
	...
	for i := range numMessages {
		wg.Go(func() {
			if err := s.Publish(ctx, top, message.Message{
				Topic: top,
